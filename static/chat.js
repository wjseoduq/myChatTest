...
$(document).ready(function() {                      // .ready document가 실행되면 한 번만 로드되는 함수
  $("#messageArea").on("submit", function(event) {  // .on id가 messageArea의 form 태그에 submit 이벤트가 발생하면 function() 함수를 실행
      ...
      $.ajax({                                      // ajax로 서버와 통신
          ...
          var botHtml = '<div class="d-flex justify-content-start mb-4"><div class="img_cont_msg"><img src="https://i.ibb.co/JdkPZ47/sesac-p3.png" class="rounded-circle user_img_msg"></div><div class="msg_cotainer">' + data + '<span class="msg_time">' + str_time + '</span></div></div>';
